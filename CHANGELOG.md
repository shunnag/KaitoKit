...
- 多言語ファイル名判定（Task B、受け入れ未完、bd `cooViewer-6lrc.1`）: 厳密復号できる 26 legacy 候補を CLDR・文字体系・正書法で採点し、非ASCII byte 数で減衰する言語事前確率を書庫全体で一度だけ適用する。文字一般カテゴリと位置、書庫全体で一つの言語を選ぶ整合、長語・タイ語の無母音・短周期反復を扱い、復号不能名も同じ byte 尺度で集計する。漢字の二重加点を撤去し、公知の頻出ハングル音節を使う。タイ語候補には字母の頻出率・稀記号・語中数字の証拠を加え、配置適合の加点を弱める。UTF-8 優先と日本語候補間の既存決定規則を保持し、正書法自己検査 CLI と自作の多言語 fixture を追加した。測定値・制約は[検証記録](Documentation/verification/2026-09-14-name-encoding-multilingual.md)を参照。
