...
- I/O バッファは大きく(256 KB)。ビットリーダは 64bit リザーバ(RAR5 が記号ごとにオフセット照会)。
...
- CP932(Shift_JIS)・EUC-JP・UTF-8 の判定を本体で持つ(UniversalDetector 相当)。Foundation `NSString.stringEncoding(for:encodingOptions:)` を候補にし、実コーパスで比較する。
